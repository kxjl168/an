function test(){
	...
	var url = getRPath()+"/manager/tcustomercomplain/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/tcustomercomplain/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/tcustomercomplain/tcustomercomplainList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
...
window.PersonnelInformationEvents_item = {
    ...
    "click #delete": function (e, value, row, index) {
        ...
        var url = getRPath() + "/manager/tcustomercomplain/delete";
...
function searchOrder() {
    var obj=$('#orderNo');
        $.ajax({
            ...
            success : function(response) {
                $('#telephone').val(response.CustomerPhone)
                $('#nickname').val(response.CustomerName)
            }
        })
    }
