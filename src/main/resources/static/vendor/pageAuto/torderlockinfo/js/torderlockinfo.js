function test(){
	...
	var url = getRPath()+"/manager/torderlockinfo/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/torderlockinfo/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/torderlockinfo/torderlockinfoList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
		...
		columns : [ {
		 ...
		 {
				field : 'orderNo',
				title : '订单编号，用于查询和前端展示(根据时间到秒+3位随机数)',
				align : 'center',
				valign : 'middle',
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/torderlockinfo/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/torderlockinfo/delete";
