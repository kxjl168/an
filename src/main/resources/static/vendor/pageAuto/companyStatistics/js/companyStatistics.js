...
function InitQuery_item() {
    ...
    $('#table_list_item').bootstrapTable({
        url : getRPath()+'/manager/companyStatistics/companyList', // 请求后台的URL（*）
        method : 'post', // 请求方式（*）
...
function item2(value, row, index) {
    return [ '<a href="javascript:void(0);" id="event2">'+(row.num2!=undefined?row.num2:0)+'</a>'].join("");
}
function item3(value, row, index) {
    return [ '<a href="javascript:void(0);" id="event3">'+(row.num3!=undefined?row.num3:0)+'</a>'].join("");
}
...
$("#btnImportOut").click(function () {
    window.open(getRPath() + '/manager/companyStatistics/companyExcel',"_blank");
});
