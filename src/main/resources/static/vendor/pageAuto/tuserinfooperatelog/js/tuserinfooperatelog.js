function test(){
	...
	var url = getRPath()+"/manager/tuserinfooperatelog/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/tuserinfooperatelog/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		...
		columns : [ {
		 ...
		 {
				field : 'operateUserId',
				title : '操作人id,外键到User表或者manager表',
				align : 'center',
				valign : 'middle',
		 ...
		 {
				field : 'auditId',
				title : '审核id，对应t_userinfo_auditid',
				align : 'center',
				valign : 'middle',
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/tuserinfooperatelog/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/tuserinfooperatelog/delete";
