  ...
  /*  $("body [id]").each(function () {
    ...
    $("#applyToLockCompanySubmit").on("click", function () {
        ...
        if (flag) {
            ...
            var url = getRPath() + "/manager/torderinfo_doing/orderAudit";
...
function InitQuery_item() {
    ...
    $('#table_list_item').bootstrapTable({
        url: getRPath() + '/manager/torderinfo_doing/torderinfoList', // 请求后台的URL（*）
        method: 'post', // 请求方式（*）
...
function initOrderAuditTable(orderId) {
    ...
    $("#orderInfoAuditTable").bootstrapTable({
        url: getRPath() + '/manager/torderinfo_doing/selectAuditByOrderId', // 请求后台的URL（*）
        method: 'post', // 请求方式（*）
        contentType: 'application/x-www-form-urlencoded',
        toolbar: '#toolbar', // 工具按钮用哪个容器
        showHeader: true,
...
function applyToLockCompanyModalShow() {
    var parameter = {}
    parameter.id = $("#auditId").val()
    var url = getRPath() + "/manager/torderinfo_doing/selectCompanyProposMoney";
    var callback = function (data) {
        if (data != 0) {
