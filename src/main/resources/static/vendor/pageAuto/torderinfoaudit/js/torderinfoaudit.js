function test(){
	...
	var url = getRPath()+"/manager/torderinfoaudit/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/torderinfoaudit/saveOrUpdate";
	...
	$('#table_list_item').bootstrapTable({
		...
		queryParams : function queryParams(params) { // 设置查询参数
			var param = {
				...
				orderInfoId : $("#q_orderInfoId").val(),
				proposer : $("#q_proposer").val(),
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/torderinfoaudit/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/torderinfoaudit/delete";
