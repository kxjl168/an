function test(){
	...
	var url = getRPath()+"/manager/tvideoalarmtalkinfo/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/tvideoalarmtalkinfo/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/tvideoalarmtalkinfo/tvideoalarmtalkinfoList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
		...
		queryParams : function queryParams(params) { // 设置查询参数
			var param = {
				...
				alarmId : $("#q_alarmId").val(),
				talkType : $("#q_talkType").val(),
		...
		columns : [ {
		 ...
		 {
				field : 'talkType',
				title : '聊天状态 1:报警人-》接警人， 2:接警人-》报警人',
				align : 'center',
				valign : 'middle',
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/tvideoalarmtalkinfo/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/tvideoalarmtalkinfo/delete";
