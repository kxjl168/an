function test(){
	...
	var url = getRPath()+"/manager/tareainfoarea/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/tareainfoarea/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/tareainfoarea/tareainfoareaList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
		...
		columns : [ {
		 ...
		 {
				field : 'districtId',
				title : '行政区域id -t_area_district-id',
				align : 'center',
				valign : 'middle',
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/tareainfoarea/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/tareainfoarea/delete";
