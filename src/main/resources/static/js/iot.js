var tipTimeout = null;
function showPopover(target, msg, type,timeminsec) {
	...
	$($(document.body).find(".tooltip")[0]).removeClass("success").removeClass(
			"error").removeClass("newmsg").removeClass("info");
	if (typeof (type) != "undefined")
		$($(document.body).find(".tooltip")[0]).addClass(type);
	else
		$($(document.body).find(".tooltip")[0]).addClass("info");
	...
	//
	var time=2000;
	if(typeof(timeminsec)!='undefined')
		time=timeminsec;
...
function datePlus (dateString, hour) {
	...
	var time = new Date(dateString.replace(/-/g,'/').replace(/T|Z/g,' ').trim());
	  ...
	  return new Date(new Date(time).valueOf() + hour * 60 * 60 * 1000)
	}
...
Date.prototype.Format = function (fmt) {
    ...
    // fmt=fmt.replace(/-/g,'/').replace(/T|Z/g,' ').trim();
    return fmt;
};
...
function myWorkbench() {
    window.location = getRPath()+"/manager/admin/index"
}
...
function newmsg(msg) {
	...
	showPopover($(target), msg, "newmsg",5000);
};
...
function error(msg,time) {
	...
	showPopover($(target), msg, "error",time);
};
