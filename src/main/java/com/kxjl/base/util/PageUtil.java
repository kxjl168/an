...
//		JSONArray rows = new JSONArray(gs.toJson(dataList));
	...
	public static <T> String packageTableDataOne(Page page, List<T> dataList) throws JSONException {
		String resultString = "";
		Gson gs = new Gson();
		JSONObject data = new JSONObject();
		JSONArray rows = JSONArray.parseArray(gs.toJson(dataList));
		data.put("total", page.getTotal());
		data.put("rows", rows);
		resultString = data.toString();
		return resultString;
	...
	public static <T> JSONObject packageTableDataObject(Page page, List<T> dataList) throws JSONException {
		JSONObject data = new JSONObject();
		data.put("total", page.getTotal());
		data.put("rows", dataList);
		return data;
